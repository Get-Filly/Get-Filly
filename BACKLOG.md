...
- [ ] **"Wanneer plaatsen"-card: verzendmoment-uitleg herbedraden** (na mig-0043) — het brein kiest bij het genereren al een goed moment + reden per kanaal (`scheduled_for` + `scheduled_reasoning` in de voorstel-prompt; `filly-brain.config.ts` met `bestHours` intact). Het gekozen moment landt op de campagne (`scheduled_for`), maar de card las z'n "Filly stelt voor + waarom"-blokje uit `suggested_scheduled_for/_reasoning` — en die kolommen worden sinds de verwijdering van `suggestSchedule` (mig-0043) niet meer gevuld → het uitleg-blokje is leeg. **Opties:** (a) `campaign-detail-adapter` de `scheduled_reasoning` van het voorstel laten tonen i.p.v. de dode `suggested_*`-kolom; (b) een expliciete "vraag een ander moment"-knop op de nieuwe detailpagina terugbouwen (her-wiren naar het brein); (c) accepteren dat het moment goed gekozen wordt maar zonder los uitleg-blokje. Bij (a)/(c) kunnen `suggested_scheduled_for/_reasoning` daarna gedropt worden (dead-on-write). Geen blocker — het moment zelf is correct.
