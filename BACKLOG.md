...
- [x] ~~**Wekelijkse interne AI-vindbaarheid-mail (Filly → Get-Filly over get-filly.com)**~~ (2026-06-08, gebouwd) — `apps/api/src/seo-report`: Vercel Cron (`apps/api/vercel.json` → `crons`, ma 07:00 UTC) → `GET /api/seo-report/run` (publiek, beveiligd met `CRON_SECRET` via `Authorization: Bearer`) → audit van get-filly.com (/, /product, /pricing, /about, /contact: title/description/canonical/og:image/JSON-LD + status van /llms.txt /robots.txt /sitemap.xml, parallelle fetches) → korte Claude-analyse (Haiku, feature `seo_weekly_audit`, restaurantId null) voor score + 3-5 kansen → HTML+text-mail via `MailService.sendSeoReport` naar info@get-filly.com. Fail-soft: zonder AI-analyse gaat het feiten-rapport tóch uit. Lean i.v.m. 10s-functielimiet. **⚠️ Vereist (Floris-actie):** `CRON_SECRET` zetten in Vercel `get-filly-api` (`openssl rand -hex 32`) + redeploy — zonder geeft het endpoint 401 en draait de cron niet.
