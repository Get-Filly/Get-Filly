...
- [ ] **Geleide flow fase 2 — context + kanalen** — `GET /suggestions/day-context?date=…` (events in de buurt + weer + aanbevolen kanalen/bereik) voedt stap 2 (event/weer bevestigen, voorgeselecteerd) + stap 3 (kanalen voorgevinkt, eigenaar past aan). `generate-for-dates` uitbreiden met optionele `channels[]` + `context`-vlaggen zodat de keuzes de generatie sturen. Stap-voor-stap onthullen met meegroeiend antwoordspoor (chips met "wijzig").
- [ ] **Geleide flow fase 3 — afronding** — antwoordspoor met "wijzig", de losse "Vraag Filly om voorstellen"-popover hierin laten opgaan (één ingang), generatie-resultaat inline in de chat tonen i.p.v. navigeren (vereist shape-adapter AiSuggestion → proposal/bundle-card), + chat-polish uit de UX-ronde (skeleton-laadstaat, slimme auto-scroll i.p.v. altijd-naar-beneden, meerregelige invoer met Shift+Enter, typing-indicator met avatar + aria-live). **Dedupe-kans:** `use-actionable-days` + UpcomingActionsBlock delen nu dezelfde rekenlogica los — samenvoegen.
