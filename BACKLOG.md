...
- [ ] **Geleide flow fase 3b — afronding** — de losse "Vraag Filly om voorstellen"-popover hierin laten opgaan (één ingang), en het generatie-resultaat inline in de chat tonen i.p.v. naar /campagnes te navigeren (vereist shape-adapter AiSuggestion → proposal/bundle-card). **Dedupe-kans:** `use-actionable-days` + UpcomingActionsBlock delen nu dezelfde rekenlogica los — samenvoegen.
