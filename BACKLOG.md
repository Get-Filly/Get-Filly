...
- [~] **Modals zonder `aria-labelledby`/focus-trap/Escape** — (✅ deels 2026-06-25, UX-ronde 3) `role="dialog"` + `aria-modal` + `aria-labelledby`/`aria-label` + Escape toegevoegd op de 3 meest-gebruikte: review-antwoord, "maak eigen campagne"-builder, media-pop-up (campagne-detail). **Rest open:** delete-modal (account), invite-modal (team), media-library-picker, history-restore. **Focus-trap** (focus binnen de modal houden) bewust nog niet — apart, vereist een gedeelde trap-helper.
