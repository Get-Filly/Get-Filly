...
- [ ] **Geleide flow fase 3b — afronding** — de losse "Vraag Filly om voorstellen"-popover hierin laten opgaan (één ingang; raakt óók de campagnes-pagina, dus productkeuze met Floris), en het resultaat als volledige interactieve approve-kaart in de chat (shape-adapter AiSuggestion → proposal/bundle-card) i.p.v. de huidige link-kaart. **Dedupe-kans:** `use-actionable-days` + UpcomingActionsBlock delen nu dezelfde rekenlogica los — samenvoegen.
- [ ] **Geleide flow fase 2b — true multi-channel** — bij meerkanaals-keuze één voorstel per kanaal (bundel) i.p.v. één voorstel op het primaire kanaal. Raakt de generatie-structuur (per-dag bundel-schema).
